...
def save_channel_file(name, cid, base_url, active_domain):
    """Her kanal için bağımsız m3u8 dosyası oluşturur."""
    safe_name = slugify(name)
    file_path = os.path.join(SAVE_FOLDER, f"{safe_name}.m3u8")
    
    content = [
        "#EXTM3U",
        f'#EXTINF:-1 group-title="int spor",{name}',
        ...
        if matches_tab:
            for a in matches_tab.find_all("a", href=re.compile(r'id=')):
                ...
                if cid_match and name_tag:
                    ...
                    save_channel_file(title, cid, base_url, active_domain)
                    ok_count += 1

        # 2. Sabit Kanallar Bölümü
        for cid, name in fixed_channels.items():
            save_channel_file(name, cid, base_url, active_domain)
            ok_count += 1

        print(f"🏁 BAŞARILI → {ok_count} kanal dosyası '{SAVE_FOLDER}' klasörüne kaydedildi.")
